...
const TableNameSysGroup = "sys_group"

// SysGroup 用户组表
type SysGroup struct {
	ID       int64     `gorm:"column:id;primaryKey;autoIncrement:true;comment:用户组唯一标识符，主键" json:"id"`          // 用户组唯一标识符，主键
	Name     string    `gorm:"column:name;comment:用户组名称" json:"name"`                                          // 用户组名称
	Code     string    `gorm:"column:code;comment:用户组编码" json:"code"`                                          // 用户组编码
	ParentID int64     `gorm:"column:parent_id;comment:父级用户组ID" json:"parent_id"`                              // 父级用户组ID
	Sort     int32     `gorm:"column:sort;comment:排序" json:"sort"`                                             // 排序
	Leader   int64     `gorm:"column:leader;comment:用户组负责人ID" json:"leader"`                                   // 用户组负责人ID
...
// TableName SysGroup's table name
func (*SysGroup) TableName() string {
	return TableNameSysGroup
}
